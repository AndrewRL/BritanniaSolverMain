...
possible_shifts = [(employee, shift) for employee in employee_data.raw_data['Name'] for shift in tour_data.raw_data['Tours']]
...
for day in days:
    for employee in employee_data.raw_data['Name']:
        britannia_model += pulp.lpSum([x[shift] for shift in [(employee, tour)
                                      for tour in tour_data.get_duties(dates=[day])]]) <= 4
